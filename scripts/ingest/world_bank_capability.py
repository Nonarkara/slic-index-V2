"""
World Bank Country-to-City Proxy Ingestion — Capability Pillar (Gini).

Maps to:
  - equal_opportunity_raw (capability, negative) ← SI.POV.GINI
    Higher Gini = more inequality = worse opportunity, so stored as-is
    (the scoring engine treats it as a negative-polarity field).

Provider: world_bank (Tier 3)
Update frequency: Annual

Usage:
    python3 scripts/ingest/world_bank_capability.py [--dry-run]
"""
...
INDICATORS = [
    {
        ...
        "label": "Equal opportunity (Gini index, country proxy)",
        "pillar": "capability",
        "source_title": "World Bank WDI: Gini index",
        "notes_prefix": "auto_ingest=world_bank_capability",
        "scale": 1.0,
        "offset": 0.0,
        "stale_before": 2015,
    },
...
def main() -> int:
    ...
    print("World Bank Capability Pillar Ingestion (Gini)")
