"""
World Bank Tourism Arrivals Ingestion — fetches international tourism arrivals,
normalises per 1 000 population, and maps to cultural_public_life_raw for every
city in the SLIC universe.

Indicators:
  - ST.INT.ARVL  (international tourism, number of arrivals)
  - SP.POP.TOTL  (total population, for per-1000 normalisation)

Maps to:
  - cultural_public_life_raw (viability, positive)

Provider: world_bank (Tier 3 — country proxy)
Update frequency: Annual

Usage:
    python3 scripts/ingest/cultural_life_proxy.py [--dry-run]
"""
...
LABEL = "Cultural & public life"
PILLAR = "viability"
SOURCE_TITLE = "World Bank WDI: International tourism arrivals per 1 000 population"
NOTES_PREFIX = "auto_ingest=cultural_life_proxy"
STALE_BEFORE = 2016
...
def main() -> int:
    ...
    print("Cultural Life Proxy Ingestion (World Bank tourism arrivals)")
    ...
    # Fetch indicators
    print("Fetching ST.INT.ARVL (international tourism arrivals)...")
    arrivals = fetch_wb_indicator("ST.INT.ARVL")
    print(f"  Got data for {len(arrivals)} countries")
    time.sleep(1)
    ...
    # Compute arrivals per 1000 population
    arrivals_per_1k: dict[str, tuple[float, int, str]] = {}
    for iso3 in arrivals:
        if iso3 not in population:
            continue
        pop_val = population[iso3][0]
        if pop_val <= 0:
            continue
        arr_val = arrivals[iso3][0]
        per_1k = round(arr_val / pop_val * 1_000, 2)
        year = arrivals[iso3][1]
        source_url = f"https://api.worldbank.org/v2/country/{iso3}/indicator/ST.INT.ARVL?format=json&mrv=10"
        arrivals_per_1k[iso3] = (per_1k, year, source_url)

    print(f"\nComputed arrivals-per-1000 for {len(arrivals_per_1k)} countries")

    total_updates = 0
    for country, cities in country_to_cities.items():
        iso3 = COUNTRY_TO_ISO3.get(country)
        if iso3 is None or iso3 not in arrivals_per_1k:
            continue

        value, year, source_url = arrivals_per_1k[iso3]
        ...
        for city_row in cities:
            ...
            if key in existing_by_key:
                ...
                row["notes"] = f"{NOTES_PREFIX}; indicator=ST.INT.ARVL; norm=per_1000_pop; year={year}; country_proxy={country}"
            else:
                ...
                new_row["notes"] = f"{NOTES_PREFIX}; indicator=ST.INT.ARVL; norm=per_1000_pop; year={year}; country_proxy={country}"
